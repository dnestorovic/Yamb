...
class ThreadSafeQueue {
 public:
  ...
  T &operator[](size_t index) { return deqQueue[index]; }

  const T &operator[](size_t index) const { return deqQueue[index]; }
  ...
  iterator end() { return {*this, deqQueue.size()}; }

  const_iterator end() const { return {*this, deqQueue.size()}; }
...
template <typename T>
class tsqueue_iterator : public std::iterator<std::bidirectional_iterator_tag,
                                              ThreadSafeQueue<T>> {
  friend class ThreadSafeQueue<T>;

 public:
  // Dereference operator returns a reference to the item contained at the
  // current position.
  T &operator*() { return q_[pos_]; }

  // Returns a const reference to the item contained at the current position.
  const T &operator*() const { return q_[pos_]; }
  ...
  tsqueue_iterator operator++(int) { return {q_, pos_++}; }
  ...
  bool operator!=(const tsqueue_iterator &lhs) const { return !(*this == lhs); }

 private:
  // Construct an tsqueue_iterator at position pos.
  tsqueue_iterator(ThreadSafeQueue<T> &q, size_t pos = 0) : q_(q), pos_(pos) {}
